...
export default function Revenue() {
  ...
  const paymentColumns = [
    ...
    { key: 'amount', header: 'Amount', render: (p: any) => <span className="text-white font-medium">{p.amount} {p.currency}</span> },
    { key: 'provider', header: 'Provider', render: (p: any) => <span className="text-gray-400">{p.provider}</span> },
    { key: 'status', header: 'Status', render: (p: any) => <StatusBadge status={p.status} /> },
    { key: 'tier', header: 'Tier', render: (p: any) => p.tier ? <StatusBadge status={p.tier} /> : <span className="text-gray-500">—</span> },
    { key: 'date', header: 'Date', render: (p: any) => <span className="text-gray-500 text-xs">{new Date(p.createdAt).toLocaleString()}</span> },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard title="Total Revenue" value={`${(revenue?.summary?.total || 0).toLocaleString()} ₽`} icon={DollarSign} color="yellow" />
        <StatCard title="Payments" value={revenue?.summary?.count || 0} icon={CreditCard} color="blue" />
        <StatCard title="Avg Payment" value={`${(revenue?.summary?.avg || 0).toFixed(0)} ₽`} icon={TrendingUp} color="emerald" />
      </div>

      {/* Revenue Chart */}
      <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
        <h3 className="text-sm font-medium text-gray-400 mb-4">Daily Revenue</h3>
      ...
      {revenue?.byPlan?.length > 0 && (
        <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
          <h3 className="text-sm font-medium text-gray-400 mb-4">Revenue by Plan</h3>
            ...
            {revenue.byPlan.map((p: any) => (
              <div key={p.tier} className="flex items-center justify-between py-2 border-b border-gray-800/50 last:border-0">
                ...
                <div className="text-right">
                  ...
                  <span className="text-gray-500 text-xs ml-2">({p.count} payments)</span>
                </div>
              </div>
            ))}
          </div>

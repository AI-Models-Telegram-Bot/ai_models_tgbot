  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Providers" value={agg.totalProviders} icon={Server} color="blue" />
        <StatCard title="Total Requests" value={agg.totalRequests} icon={Zap} color="purple" />
        <StatCard
          title="Est. Total Spend"
        ...
        <StatCard
          ...
          icon={Activity}
          color="emerald"
          trend={{ value: agg.overallSuccessRate, label: '% success' }}
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-400">OpenRouter Credits</h3>
            {openRouterBalance?.status === 'ok' && (
              <span className="text-xs bg-emerald-600/20 text-emerald-400 px-2 py-0.5 rounded-full">Connected</span>
            )}
          </div>
          {balancesLoading ? (
            ...
            <div className="text-sm text-red-400">{openRouterBalance?.error || 'No API key configured'}</div>
          )}
        </div>
        ...
        <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-400">ElevenLabs Characters</h3>
          ...
          {balancesLoading ? (
            ...
            <div className="text-sm text-red-400">{elevenLabsBalance?.error || 'No API key configured'}</div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Cost Pie Chart */}
        <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
          <h3 className="text-sm font-medium text-gray-400 mb-4">Cost by Provider</h3>
        ...
        <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800">
          <h3 className="text-sm font-medium text-gray-400 mb-4">Circuit Breakers</h3>
          {cbEntries.length === 0 ? (
                  ...
                  <div className="text-xs">
                    {state.isOpen ? (
                      <span className="text-red-400">
                        OPEN ({Math.ceil(state.cooldownRemaining / 1000)}s left)
                      </span>
                    ) : state.failures > 0 ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="text-right px-4 py-3 text-gray-500 font-medium">Success %</th>
                <th className="text-right px-4 py-3 text-gray-500 font-medium">Avg Cost</th>
                <th className="text-right px-4 py-3 text-gray-500 font-medium">Avg Time</th>
                <th className="text-right px-4 py-3 text-gray-500 font-medium">Total Cost</th>
              </tr>
            </thead>
            <tbody>
